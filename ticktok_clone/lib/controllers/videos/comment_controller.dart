...
class CommentController extends GetxController{ 
  ...
  postComment({
    ...
    try {
      ...
      CommentModel commentModel = CommentModel(username: (userDoc.data() as dynamic )["username"], comment: commentText, datePublished: DateTime.now(), likes: [], profilePhoto: (userDoc.data() as dynamic )["imagUrl"], uid: auth.currentUser!.uid, id: commentID);

      await firestore.collection("videos").doc(_postID).collection("comments").doc(commentID).set(commentModel.toJson());

      // update comment count in video collection
     DocumentSnapshot comDoc = await firestore.collection("videos").doc(_postID).get();
      ...
      Get.snackbar(e.toString(), "Some Error occurred while Commting to the Video");
    }
